...
export class AuthFactory {
  ...
  async updateBasicInfoAndEmail(updateBasicInfoDTO: UpdateBasicInfoDTO) {
    const user: any = {};

    if (updateBasicInfoDTO.fullName)
      user.fullName = updateBasicInfoDTO.fullName;

    if (updateBasicInfoDTO.gender) user.gender = updateBasicInfoDTO.gender;

    if (updateBasicInfoDTO.email) user.email = updateBasicInfoDTO.email;
